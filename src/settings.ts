...
export interface CloudSyncSettings {
  ...
  vaultName: string;
  /**
   * When true, auto-downloads are deferred for any file that is currently
   * open in an Obsidian editor while the user is actively typing.
   * Prevents sync from overwriting in-progress edits.
   */
  typingProtection: boolean;
  /** Check GitHub for a newer plugin version on startup and offer to install it. */
  autoUpdate: boolean;
}
  ...
  async display(): Promise<void> {
    ...
    if (isLoggedIn) {
      ...
      containerEl.createEl("p", {
        text: "All devices connected to your account. Device names include the plugin version so you can see which ones need updating.",
        attr: { style: "color: var(--text-muted); margin-bottom: 8px; font-size: 0.9em;" },
      });

      let devices: import("./api").DeviceInfo[] = [];
      try {
        devices = await this.plugin.api.listDevices();
      } catch {
        containerEl.createEl("p", { text: "Could not load device list (server unreachable).", cls: "cloudsync-chip-empty" });
      }

      for (const device of devices.filter(d => !d.revoked)) {
        const isThisDevice = device.id === this.plugin.settings.deviceId;
        const lastSeen = this.timeAgo(device.last_seen_at * 1000);
        const setting = new Setting(containerEl)
          .setName(device.name + (isThisDevice ? " (this device)" : ""))
          .setDesc(`Last seen: ${lastSeen}`);
            ...
            btn
              ...
              .onClick(async () => {
                try {
                  ...
                  new Notice(`CloudSync: Revoked "${device.name}"`);
                  this.display();
                } catch (e: unknown) {
                  const msg = e instanceof Error ? e.message : String(e);
                  new Notice(`CloudSync: Could not revoke device — ${msg}`);
                }
              })
          );
        }
    ...
    new Setting(containerEl)
      .setName("Typing protection")
      .setDesc(
        "Defer downloads for files you're actively editing. " +
        "When you stop typing, pending downloads flush automatically. " +
        "Prevents sync from overwriting in-progress edits — highly recommended."
      )
      .addToggle((toggle) =>
        toggle
          .setValue(this.plugin.settings.typingProtection)
          .onChange(async (value) => {
            this.plugin.settings.typingProtection = value;
            await this.plugin.saveSettings();
          })
      );
    ...
    new Setting(containerEl)
      .setName("Auto-update")
      .setDesc(
        "Check GitHub for a newer plugin version on startup and offer a one-click install. " +
        "The update is written to the plugin folder and takes effect after reloading."
      )
      .addToggle((toggle) =>
        toggle
    ...
    new Setting(containerEl)
      .setName(`Installed version: v${this.plugin.manifest.version}`)
      .setDesc("Checks the GitHub releases page for the latest version.")
      .addButton((btn) =>
